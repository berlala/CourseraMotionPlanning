...
# State machine states 状态机的3个状态
FOLLOW_LANE = 0
DECELERATE_TO_STOP = 1
STAY_STOPPED = 2
# Stop speed threshold
STOP_THRESHOLD = 0.02
# Number of cycles before moving from stop sign. STOP Sign的停车时长
STOP_COUNTS = 10
...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            print("=== === Behavior Closest Index is "+str(closest_index))
            # ------------------------------------------------------------------
            ...
            print(" === === Behavior Goal Index is "+str(goal_index))
            # ------------------------------------------------------------------
            ...
            print("=== === Behavior Stop Index is "+str(goal_index))
            # ------------------------------------------------------------------
            ...
            # ------------------------------------------------------------------
            if stop_sign_found:
                self._goal_state[2] = 0.0 # 改goal state的目标速度为0m/s
                self._state  = DECELERATE_TO_STOP
                print("=== === Change to STATE: DECELERATE_TO_STOP === === ")
            #   ...
            # ------------------------------------------------------------------
        ...
        # state.
            ...
            if(closed_loop_speed<=STOP_THRESHOLD):
                print("closed_loop_speed is "+ str(closed_loop_speed))
                self._state = STAY_STOPPED
                print("=== === Change to STATE: STAY_STOPPED === ===")
            # ------------------------------------------------------------------
        ...
        # the stop sign and transition to the next state.
            ...
            # You should use the get_closest_index(), get_goal_index(), and 
            # check_for_stop_signs() helper functions.
            print('current loc x '+str(ego_state[0]))
            print('current loc y '+str(ego_state[1]))
            if self._stop_count == STOP_COUNTS:
                ...
                # 这个操作的意思是即使已经起步了，只要前面还有stop sign，就还处于STAY_STOPPED状态？
                self._goal_index = goal_index # lookahead的目标
                self._goal_state = waypoints[goal_index]
                # --------------------------------------------------------------
                ...
                # --------------------------------------------------------------
                    self._state = FOLLOW_LANE
                    print("=== === Change to STATE: FOLLOW_LANE === ===")
                pass
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # 画图可以帮助理解，这个地方wp_index直接影响停止线距离（waypoint的散点间隔较大）
        # ------------------------------------------------------------------
        while wp_index < len(waypoints) - 1:
            wp_index += 1
            arc_length += math.sqrt((waypoints[wp_index][0]-waypoints[wp_index-1][0])*(waypoints[wp_index][0]-waypoints[wp_index-1][0])+\
                             (waypoints[wp_index][1]-waypoints[wp_index-1][1])*(waypoints[wp_index][1]-waypoints[wp_index-1][1]))
            if arc_length > self._lookahead:
                # wp_index += 1
                break

        wp_index = min(len(waypoints) - 1, wp_index)
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        cur_distance = math.sqrt((waypoints[i][0] - ego_state[0])*(waypoints[i][0] - ego_state[0]) +\
                                 (waypoints[i][1] - ego_state[1])*(waypoints[i][1] - ego_state[1]))
        distance_list.append(cur_distance)

    closest_len = min(distance_list)
    closest_index = distance_list.index(closest_len)
    # ------------------------------------------------------------------
