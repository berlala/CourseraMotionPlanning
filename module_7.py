    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if frame % LP_FREQUENCY_DIVISOR == 0:
                ...
                print('Goal set has ' + str(len(goal_state_set)))

                #  # Calculate planned paths in the local frame.
                # 在内部可以选择求解螺旋先参数的形式[1]优化的方法，软化终点约束，求曲率最小; [2]求非线性方程组
                ...
                print('Paths has ' + str(len(paths)))
                #for i in range(np.size(collision_check_array)):
                #    print('col check result '+str(collision_check_array[i]))
