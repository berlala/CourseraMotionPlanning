...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        Assumptions:
            1. The first point in the spiral is in origin of the vehicle frame 起点是坐标系原点
        ...
        # arc length to the goal.任何长度不可能小于sf_0
        ...
        # ------------------------------------------------------------------
        para_bounds = scipy.optimize.Bounds([-0.5,-0.5,sf_0], [0.5,0.5,inf], keep_feasible=False)
        # ------------------------------------------------------------------
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(fun=self.objective, x0=p0, args=(), method='L-BFGS-B', jac=self.objective_grad, hess=None, hessp=None, bounds=para_bounds, constraints=(), \
                                        tol=None, callback=None, options=None)
        # ------------------------------------------------------------------

        spiral,kappas = self.sample_spiral(res.x)
        kappa_exceed = False
        if max(abs(max(kappas)),abs(min(kappas))) > 1.0: # 曲率约束是否超限
            kappa_exceed = True
        return spiral, kappa_exceed


    def solve_spiral_equation(self, xf, yf, tf):
        """根据首尾点条件，求解非线性等式方程，得到螺旋线参数,该螺旋线是否曲率超限（限制0.5）
        """
        self._xf = xf
        self._yf = yf
        self._tf = tf
        ...
        def f(X):
            ...
            a3 = X[2]
            sf = X[3]
            return [a3*sf**3 + a2*sf**2 + a1*sf,    # theta(0) = 0
                    a3*sf**4/4 + a2*sf**3/3 + a1*sf**2/2 - tf,  # theta(sf) = tf
                    sf/24*(  cos(0) +  \
                            4* cos(a3*sf**4/16384.0 + a2*sf**3/1536.0 + a1*sf**2/128.0) +  \
                            2* cos(a3*(2*sf)**4/16384.0 + a2*(2*sf)**3/1536.0 + a1*(2*sf)**2/128.0) + \
                            4* cos(a3*(3*sf)**4/16384.0 + a2*(3*sf)**3/1536.0 + a1*(3*sf)**2/128.0) + \
                            2* cos(a3*(4*sf)**4/16384.0 + a2*(4*sf)**3/1536.0 + a1*(4*sf)**2/128.0) + \
                            4* cos(a3*(5*sf)**4/16384.0 + a2*(5*sf)**3/1536.0 + a1*(5*sf)**2/128.0) + \
                            2* cos(a3*(6*sf)**4/16384.0 + a2*(6*sf)**3/1536.0 + a1*(6*sf)**2/128.0) + \
                            4* cos(a3*(7*sf)**4/16384.0 + a2*(7*sf)**3/1536.0 + a1*(7*sf)**2/128.0) + \
                            1* cos(a3*(8*sf)**4/16384.0 + a2*(8*sf)**3/1536.0 + a1*(8*sf)**2/128.0) ) - xf, # x(sf) = xf
                    sf/24*(  sin(0) +  \
                            4* sin(a3*sf**4/16384.0 + a2*sf**3/1536.0 + a1*sf**2/128.0) +  \
                            2* sin(a3*(2*sf)**4/16384.0 + a2*(2*sf)**3/1536.0 + a1*(2*sf)**2/128.0) + \
                            4* sin(a3*(3*sf)**4/16384.0 + a2*(3*sf)**3/1536.0 + a1*(3*sf)**2/128.0) + \
                            2* sin(a3*(4*sf)**4/16384.0 + a2*(4*sf)**3/1536.0 + a1*(4*sf)**2/128.0) + \
                            4* sin(a3*(5*sf)**4/16384.0 + a2*(5*sf)**3/1536.0 + a1*(5*sf)**2/128.0) + \
                            2* sin(a3*(6*sf)**4/16384.0 + a2*(6*sf)**3/1536.0 + a1*(6*sf)**2/128.0) + \
                            4* sin(a3*(7*sf)**4/16384.0 + a2*(7*sf)**3/1536.0 + a1*(7*sf)**2/128.0) + \
                            1* sin(a3*(8*sf)**4/16384.0 + a2*(8*sf)**3/1536.0 + a1*(8*sf)**2/128.0) ) - yf    # y(sf) = yf     
                    ]

        result = scipy.optimize.fsolve(f,X0)

        spiral,kappas = self.sample_spiral_parameters(result)
        kappa_exceed = False
        if max(abs(max(kappas)),abs(min(kappas))) > 1.0:  # 曲率约束是否超限
            kappa_exceed = True
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        # # Remember that a, b, c, d and s are lists  局部坐标系
        # ...
        thetas = []
        #for i in range(len(s)):
        thetas.append(a*s + b*s**2/2.0 + c*s**3/3.0 + d*s**4/4.0)
        return thetas
        # ------------------------------------------------------------------

    def kappaf(self, a, b, c, d, s):
        pass
        #for i in range(len(s)):
        kappa = a + b*s**2 + c*s**3+ d*s**4
        return kappa
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        s_points = np.linspace(0.0, p[4])#按default个数返回一系列点
        ...
        # Try to vectorize the code using numpy functions for speed if you can.
        # 辛普森积分计算kappa,theta-> x,y变换, 局部坐标系x0,y0 = 0
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        t_points = []
        x_points = []
        ...
        for i in range(len(s_points)):
            cur_s = s_points[i]
            theta_0 = self.thetaf( a, b, c, d, 0)
            theta_1 = self.thetaf( a, b, c, d, cur_s/8.0)
            theta_2 = self.thetaf( a, b, c, d, cur_s/8.0*2)
            theta_3 = self.thetaf( a, b, c, d, cur_s/8.0*3)
            theta_4 = self.thetaf( a, b, c, d, cur_s/8.0*4)
            theta_5 = self.thetaf( a, b, c, d, cur_s/8.0*5)
            theta_6 = self.thetaf( a, b, c, d, cur_s/8.0*6)
            theta_7 = self.thetaf( a, b, c, d, cur_s/8.0*7)
            theta_8 = self.thetaf( a, b, c, d, cur_s/8.0*8)
            x_point = cur_s/24.0*(cos(theta_0[0]) + 4*cos(theta_1[0])+ 2*cos(theta_2[0])+ 4*cos(theta_3[0])+ 2*cos(theta_4[0])+\
             4*cos(theta_5[0])+ 2*cos(theta_6[0])+ 4*cos(theta_7[0])+ cos(theta_8[0]))
            y_point = cur_s/24.0*(sin(theta_0[0]) + 4*sin(theta_1[0])+ 2*sin(theta_2[0])+ 4*sin(theta_3[0])+ 2*sin(theta_4[0])+\
             4*sin(theta_5[0])+ 2*sin(theta_6[0])+ 4*sin(theta_7[0])+ sin(theta_8[0]))
            t_point = theta_8[0]
            kappa_point = self.kappaf(a, b, c, d, cur_s) 

            x_points.append(x_point)
            y_points.append(y_point)
            t_points.append(t_point)
            kappa_points.append(kappa_point)

        return [x_points, y_points, t_points], kappa_points
        # ------------------------------------------------------------------

    def sample_spiral_parameters(self, result_parameters):
        """Samples a set of points along the spiral given the optimization
        parameters.
        """
        ...
        c = result_parameters[1]
        d = result_parameters[2]
        sf = result_parameters[3]

        # Set the s_points (list of s values along the spiral) to be from 0.0
        # to p[4] (final arc length)
        s_points = np.linspace(0.0, sf)#按default个数返回一系列点
