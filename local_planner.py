...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        heading = np.arctan2(delta_y,delta_x) # 注意顺序
        # ------------------------------------------------------------------
        ...
        # The local goal state will be of the form (x, y, t, v). goal_state只有x,y,v
    ...
    def plan_paths(self, goal_state_set):
        ...
        for goal_state in goal_state_set:
            curvature_exceed = False
            #  ===========================================================
            # [1] 优化的方法求螺旋线参数方程系数
            path,curvature_exceed = self._path_optimizer.optimize_spiral(goal_state[0], 
                                                        goal_state[1], 
                                                        goal_state[2])
            # [2] 求非线性方程的方法求螺旋线参数方程系数
            # path,curvature_exceed = self._path_optimizer.solve_spiral_equation(goal_state[0], 
            #                                                   goal_state[1], 
            #                                                   goal_state[2])
            # ============================================================
            if np.linalg.norm([path[0][-1] - goal_state[0], 
                               path[1][-1] - goal_state[1], 
                               path[2][-1] - goal_state[2]]) > 0.1 or curvature_exceed is True :
